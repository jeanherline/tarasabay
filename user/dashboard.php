          ...
          <div class="row">
            <div class="col-xl-3 col-sm-6 mb-3">
              <div class="card text-white" style="background-color: #EAAA00;">
                ...
                <a class="card-footer text-white clearfix small z-1" href="https://tarasabay.000webhostapp.com/registeredCars.php">
            ...
            <div class="col-xl-3 col-sm-6 mb-3">
              <div class="card text-white" style="background-color: #EAAA00;">
                ...
                <a class="card-footer text-white clearfix small z-1" href="https://tarasabay.000webhostapp.com/pendingCars.php">
                  <span class="float-left">View Details</span>
                  <span class="float-right">
                    <i class="fas fa-angle-right"></i>
                  </span>
                ...
                <table class="table table-bordered table-striped table-hover" id="dataTable" width="100%" cellspacing="0">
                  ...
                  <tbody>
                    ...
                    $ret = "SELECT c.car_id, c.owner_id, c.brand, c.model, ci.car_identity_num, ci.cr_number, ci.or_number, ci.reg_exp_date, c.status, c.color, c.seat_count
                      ...
                      JOIN car_identification ci ON c.car_id = ci.car_id
                      JOIN user_profile up ON c.owner_id = up.user_id
                      WHERE c.status = 'pending' AND c.owner_id = ?
                      ORDER BY c.owner_id";
                    $stmt = $db->prepare($ret);
                    $stmt->bind_param("i", $id); // Assuming $id contains the specific user_id
                    $stmt->execute();
                    $result = $stmt->get_result();
                    $cnt = 1;
                    while ($row = $result->fetch_assoc()) {
